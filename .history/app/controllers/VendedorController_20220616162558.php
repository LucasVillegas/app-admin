...
class VendedorController
{
    ...
    public function listar_vendedor()
    {
        if ($this->helpers->limpiar_cadena(isset($_POST['dato'])) && $this->helpers->limpiar_cadena(!empty($_POST['dato']))) {
            ...
            $sql = $this->conexion->ejecutar_consulta_simple("SELECT *,p.id AS persona_id,u.id AS users_id FROM personas p INNER JOIN usuarios u ON u.persona_id=p.id WHERE u.tipo_usuario='Vendedor' WHERE p.nombre LIKE '%$search%' ");
            $resultado = $sql->fetchAll();
            $json = array();
            foreach ($resultado as $row) {
                $json[] = array(
