...
class RutasControllers
{
    ...
    {
        $id = $thiss->helpers->limpiar_cadena($_POST['id']);
        $dia = $thiss->helpers->limpiar_cadena($_POST['dia']);
        $ruta_id = $thiss->helpers->limpiar_cadena($_POST['ruta']);
        $datos = [
            "Dia" => $dia,
            "Ruta" => $ruta_id,
            "Id" => $id,
        ];
        $guardar = $thiss->modelo->updatedia($datos);
        $respuesta = 0;
        if ($guardar->rowCount() >= 1) {
            $respuesta++;
        }
