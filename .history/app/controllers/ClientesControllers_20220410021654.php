...
class ClientesControllers
{
    ...
    public function listar_clientes()
    {
        ...
        WHERE vendedores.id='$vendedor_id' AND clientes.nombre_tienda LIKE '%$search%' 
        OR clientes.nombre_cliente LIKE '%$search'LIMIT 10");
        ...
        INNER JOIN rutas ON rutas.id=clientes.ruta_id
        INNER JOIN vendedores ON vendedores.id=rutas.vendedor_id
        WHERE vendedores.id='$vendedor_id' LIMIT 10");
            $resultado = $sql->fetchAll();
            $json = array();
            foreach ($resultado as $row) {
                $json[] = array(
