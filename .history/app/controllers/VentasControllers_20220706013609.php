...
class VentasControllers
{
    ...
    public function Listar_ventas_anexos()
    {
        ...
        $id = $this->helpers->limpiar_cadena($_POST['idv']);
        $sql = $this->conexion->ejecutar_consulta_simple("SELECT p.id,p.codigo_producto,p.precio,p.nombre_producto,u.nombre_unidad,p.precio_compra,dv.cantidad_venta
        FROM ventas v
        INNER JOIN detalle_ventas dv ON v.id=dv.id_detalle_venta
        INNER JOIN clientes c ON v.cliente_id=c.id
        INNER JOIN productos p ON dv.productos=p.id
        INNER JOIN unidades u ON p.unidad_id=u.id
        WHERE v.id='$id' AND v.usuario_id='$usuario'");
        $resultado = $sql->fetchAll();
        $json = array();
        foreach ($resultado as $row) {
            $json[] = array(
