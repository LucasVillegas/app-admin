...
class RutasControllers
{
    ...
    public function Lista_Rutas()
    {
        if ($this->helpers->limpiar_cadena(isset($_POST['dato'])) && $this->helpers->limpiar_cadena(!empty($_POST['dato']))) {
            ...
            $sql = $this->conexion->ejecutar_consulta_simple("SELECT *, r.id AS ruta_id  FROM rutas r INNER JOIN vendedores v ON r.vendedor_id=v.id INNER JOIN personas p ON v.persona_id=p.id WHERE nombre_unidad LIKE '%$search%' ");
            ...
            $sql = $this->conexion->ejecutar_consulta_simple("SELECT * FROM unidades LIMIT 15");
            $resultado = $sql->fetchAll();
            $json = array();
            foreach ($resultado as $row) {
                $json[] = array(
