...
class ClientesControllers
{
    ...
    public function preparar_cliente()
    {
    ...
    public function actualizar_cliente()
    {
        ...
        $guardar = $this->modelo->update_cliente($datos);
        $respuesta = 0;
        if ($guardar->rowCount() >= 1) {
            $respuesta++;
        }
    ...
    public function block()
    {
        ...
        $eliminar = $this->modelo->blockcliente($datoseli);
        $cont = 0;
    ...
    public function active()
    {
        ...
        $eliminar = $this->modelo->activecliente($datoseli);
        $cont = 0;
        if ($eliminar->rowCount() >= 1) {
            $cont++;
        }
    ...
    {
        $id = $this->helpers->limpiar_cadena($_POST['Codcli']);
        $datoseli = [
            'Id' => $id,
        ];
        $venta = $this->conexion->ejecutar_consulta_simple("SELECT cliente_id FROM ventas WHERE cliente_id='$id'");
        if ($venta->rowcount() >= 1) {
            echo json_encode(0);
        } else {
            $eliminar = $this->cliente->deletecliente($datoseli);
            $cont = 0;
            if ($eliminar->rowCount() >= 1) {
                $cont++;
            }
