...
class ProductosControllers
{
    ...
    public function agregar_producto()
    {
        ...
        $r                    = rand(0, 1000);
        $fecha                = date("his");
        $ruta_guardar_archivo = $ruta_carpeta . basename($this->helpers->limpiar_cadena($_FILES['file']['name']));
        $nombre_archivo       = basename($_FILES['file']['name']);

        if (move_uploaded_file($this->helpers->limpiar_cadena($_FILES['file']['tmp_name']), $ruta_guardar_archivo)) {
