        ...
        if (e.target.matches('.form-open-modal')) {
            ...
            if (id_venta != 0) {
                ...
                data.detalle.forEach(v => {
                    ...
                    templatedetalle += `
                            <h6 class="nombre-codigo">${v.nombre} - ${codigo}</h6>
                            ...
                            <h6 class="total-producto text-right" style="font-size:12px;margin-top:-25px;">$ ${formatNum(v.precio_venta * v.cantidad_venta)}</h6>
                            <hr class="my-1">
                            `;
                });
                ...
                data1.venta.forEach(c => {
                    ...
                    //d.getElementById('venta-id').value = c.id;
                    // d.getElementById('div-btn').innerHTML = '<a href="#" data-toggle="modal" data-target="#modal-anexar" class="btn btn-dark btn-sm btn-block btn-anexar" id="btn-anexar"><i class="fas fa-cart-plus"></i> Anexar Productos </a>';

                    templateventa += `
                        <h4 class="estado"><span class="badge ${c.estado_venta == 1 ? "text-warning" : "text-success"}">${c.estado_venta == 1 ? "Pendiente de facturación" : "Facutación aprobada"}</span></h4>
                        <h6 class="factura text-dark">${c.id}</h6>
                        <h6 class="fecha text-dark" style="margin-top:-10px;">${c.fecha_entrega}</h6>
                        <h6 class="cliente" style="margin-top:-10px;">${c.nombre_cliente} / ${c.nombre_tienda}</h6>
                        <h5 class="total">$ ${formatNum(c.total_venta)}</h5>
                        `;
                });
                d.getElementById('venta').innerHTML = templateventa;
                /* $.post(URL.API_DETALLE_VENTAS, data, function (response) {
                    ...
                    venta.detalle.forEach(v => {
                        ...
                        templatedetalle += `
                            <h6 class="nombre-codigo">${v.nombre} - ${codigo}</h6>
                            <h6 class="precio text-dark" style="margin-top:-10px;">$ ${formatNum(v.precio_venta)}/${v.descripcion_unidad}</h6>
                            <h6 class="cantidad text-center" style="margin-top:-20px;">X${v.cantidad_venta}</h6>
                            <h5 class="total-producto text-right" style="margin-top:-25px;">$ ${formatNum(v.precio_venta * v.cantidad_venta)}</h5>
                            <hr class="my-1">
                            `;
                    });
                    d.getElementById('detalle').innerHTML = templatedetalle;
                ...
                /* $.post(URL.API_DETALLE_VENTAS_CABECERAS, data, function (response) {
                    let venta = JSON.parse(response);
                    let templateventa = "";
                ...
                /*                 let data = await ajax(URL.API_LISTAR_VENTAS_DETALLE, {
                                ...
                                d.getElementById("obs").textContent = data.forEach((value) => { observaciones });
                                data.forEach((value) => {
                                    let { nombre_producto, codigo_producto, nombre_unidad, precio, cantidad, observaciones } = value;
                                    if (!data.length) {
                                        ...
                                        document
                                            .getElementById("detalle-producto")
                                            .content.querySelector(".nombre-codigo").textContent = nombre_producto + " - " + codigo;
                                        document
                                            .getElementById("detalle-producto")
                                            .content.querySelector(".precio").textContent = precio + " / " + nombre_unidad;
                                        document
                                            .getElementById("detalle-producto")
                                            .content.querySelector(".cantidad").textContent = "X" + cantidad;
                                        total_producto = formatNum(precio * cantidad);
                                        document
                                            .getElementById("detalle-producto")
                                            .content.querySelector(".total-producto").textContent = "$" + total_producto;
                                        document
                                            .getElementById("detalle-producto")
                                            .content.querySelector(".obs").textContent = observaciones;
                                        let clone = document.importNode(
                                            document.getElementById("detalle-producto").content,
                                            true
                                        );
