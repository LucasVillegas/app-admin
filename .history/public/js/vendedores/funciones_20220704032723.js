...
export function requests() {
    d.addEventListener("submit", async (e) => {
        ...
        if (e.target.matches('#form-save')) {
            if (d.getElementById('id').value == "" && d.getElementById('id_vendedor_user').value == "") {
                let data = await ajax(URL.API_AGREGAR_VENDEDOR, {
                    method: "POST",
                    body: appendForm({
                        form: d.createElement("form"),
                        identificacion: d.getElementById('identificacion').value,
                        nombre: d.getElementById('nombres').value,
                        apellido: d.getElementById('apellidos').value,
                ...
                if (data == 1) {
                    Toast.fire({
                        icon: 'success',
                        title: 'Vendedor Registrado Exitosamente'
                    });
                    listar_vendedores("");
                    ...
                    Toast.fire({
                        icon: 'warning',
                        title: 'La identificacion ya se encuentra registrada en el sitema.'
                    });
                } else if (data == 2) {
                    Toast.fire({
                        icon: 'warning',
                        title: 'El Correo ya se encuentra registrado en el sitema.'
                    });
                } else if (data == 3) {
                    Toast.fire({
                        icon: 'warning',
                        title: 'El Usuario ya se encuentra registrado en el sitema.'
                    });
                } else {
                    Toast.fire({
                        icon: 'error',
                        title: 'Vendedor no Agregado'
                    });
                }
        ...
        if (e.target.matches('#form-bloq')) {
            let padre = e.target.parentElement.parentElement;
            let id_vendedores = padre.querySelector(".id_vendedores").value;
            Swal.fire({
